...
class CAN_TP:
    def __init__(self, tx_id, channel, baud_rate, message_type, event_manager: EventManager) -> None:
        self.event_manager = event_manager
        self.can = CAN(tx_id, channel, baud_rate, message_type, self.event_manager)
    ...
    def process_frame(self, incoming_frame):
        ...
        if self.frame_type == self.frame.SINGLE_FRAME:
            ...
            self.frames_received = 0  # We've received the first frame

            # Initialize counter and store data
            self.counter = min(self.no_of_frames, self.block_size)  # Subtract 1 as we've already received the first frame
