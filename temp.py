...
class CAN_TP:
    ...
    def process_frame(self, incoming_frame):
        ...
        if self.frame_type == self.frame.SINGLE_FRAME:
            ...
            self.block_size = incoming_frame[1]
            self.time_between_consecutive_frames = incoming_frame[2]
            print(f"Flow control frame received: block size = {self.block_size}, time between frames = {self.time_between_consecutive_frames} ms")
            self.send_consecutive_frames()
    ...
    def send_consecutive_frames(self):
        ...
        if not self.remaining_data and self.block_size > 0:
